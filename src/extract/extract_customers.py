...
EXTRACT_CUSTOMERS_QUERY_FILE = os.path.join(
    os.path.dirname(__file__), "../sql/extract_customers.sql"
)

EXPECTED_IMPORT_RATE = 0.001

TYPE = "CUSTOMERS from pagila database"
...
def extract_customers() -> pd.DataFrame:
    try:
        ...
        customers = extract_customers_execution()
...
def extract_customers_execution() -> pd.DataFrame:
    # Import the SQL query
    connection_details = load_db_config()["source_database"]
    print(connection_details)
    query = import_sql_query(EXTRACT_CUSTOMERS_QUERY_FILE)
