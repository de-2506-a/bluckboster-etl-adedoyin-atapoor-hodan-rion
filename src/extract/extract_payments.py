...
EXTRACT_PAYMENTS_QUERY_FILE = os.path.join(
    os.path.dirname(__file__), "../sql/extract_payments.sql"
)

EXPECTED_IMPORT_RATE = 0.001

TYPE = "PAYMENTS from pagila database"
...
def extract_payments() -> pd.DataFrame:
    try:
        ...
        payments = extract_payments_execution()
        extract_payments_execution_time = (
...
def extract_payments_execution() -> pd.DataFrame:
    # Import the SQL query
    connection_details = load_db_config()["source_database"]
    print(connection_details)
    query = import_sql_query(EXTRACT_PAYMENTS_QUERY_FILE)
