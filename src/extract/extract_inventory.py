...
EXTRACT_INVENTORY_QUERY_FILE = os.path.join(
    os.path.dirname(__file__), "../sql/extract_inventory.sql"
)

EXPECTED_IMPORT_RATE = 0.001

TYPE = "INVENTORY from pagila database"
...
def extract_inventory() -> pd.DataFrame:
    try:
        ...
        inventory = extract_inventory_execution()
...
def extract_inventory_execution() -> pd.DataFrame:
    # Import the SQL query
    connection_details = load_db_config()["source_database"]
    print(connection_details)
    query = import_sql_query(EXTRACT_INVENTORY_QUERY_FILE)
    ...
    inventory_df.to_csv("data/processed/cleaned-inventory.csv", index=False)
    connection.close()
